...
public final class DiscordSyncModule {
    ...
    public void handleUnauthorizedCommand(Player player, String command) {
        ...
        if (plugin.isDiscordEnabled()) {
            plugin.getDiscord().sendEmbed("discord-sync-alert", Map.of(
                    "player", player.getName(), "command", command, "action", unauthorizedAction.toUpperCase(Locale.ROOT)), false);
        }
        switch (unauthorizedAction.toLowerCase(Locale.ROOT)) {
            case "shutdown", "stop" -> plugin.getSchedulerService().runGlobalDelayed(plugin.getServer()::shutdown, 40L);
    ...
    private static Set<Long> parseIds(List<String> raw) {
        Set<Long> result = new HashSet<>();
        for (String value : raw) {
            try { result.add(Long.parseUnsignedLong(value.trim())); }
