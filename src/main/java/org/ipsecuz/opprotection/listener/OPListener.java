...
public final class OPListener implements Listener {
    ...
    public void onItemConsume(PlayerItemConsumeEvent event) {
        // Consume covers potions/milk/food: without it a locked admin could clear potion
        // effects (milk) or gain effects while still unverified.
        if (restricted(event.getPlayer())) event.setCancelled(true);
    }

    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    public void onTeleport(PlayerTeleportEvent event) {
        // Blocks ender pearls, portals and plugin teleports that would otherwise move a
        // locked player out of the verification area.
        if (restricted(event.getPlayer())) event.setCancelled(true);
    }
