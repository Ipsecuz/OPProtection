...
public final class OpManager implements Listener {
    /** Vanilla-safe Minecraft username; used to make console 'ban' dispatches injection-proof. */
    ...
    /** Dedup guard so logout-actions run exactly once per session (timeout path + quit path). */
    private final Set<UUID> logoutActionsExecuted = ConcurrentHashMap.newKeySet();
    ...
    public void reload(Set<String> opWhitelist, String opPassword, int passTimeout,
        ...
        // Cap at 120 minutes (144000 ticks) to prevent excessively long sessions
        this.verificationResetTicks = Math.min(144_000L, Math.max(1L,
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        checkIpChangeAndAlert(player);
        plugin.getSchedulerService().runEntityDelayed(player, () -> inspectPrivilege(player), 1L);
    }

    /**
     * Check if player's current IP differs from the stored trusted IP.
     * If different and Discord IP-change-alert is enabled, send a Discord alert.
     */
    private void checkIpChangeAndAlert(Player player) {
        if (player == null || !player.isOnline()) return;
        if (!plugin.isDiscordEnabled()) return;

        org.bukkit.configuration.file.FileConfiguration config = plugin.getConfig();
        boolean alertEnabled = config.getBoolean("discord.ip-change-alert.enabled", false);
        if (!alertEnabled) return;

        boolean whitelistOnly = config.getBoolean("discord.ip-change-alert.whitelist-only", true);
        if (whitelistOnly && !isWhitelisted(player.getName())) return;

        String currentIp = ip(player);
        String trustedIp = plugin.getSecurityDataStore().getTrustedIp(player.getUniqueId());
        if (trustedIp == null || trustedIp.isBlank() || trustedIp.equals("unknown")) return;
        if (trustedIp.equals(currentIp)) return;

        // IP changed — send Discord alert
        String alertRoleId = config.getString("discord.ip-change-alert.alert-role-id", "");
        String playerName = player.getName();
        plugin.getSchedulerService().runGlobal(() -> {
            if (plugin.isDiscordEnabled()) {
                plugin.getDiscord().sendIpChangeAlert(playerName, currentIp, trustedIp, alertRoleId);
                plugin.getAuditLog().write("IP_CHANGE_ALERT", playerName,
                        player.getUniqueId().toString(), currentIp,
                        "trustedIp=" + trustedIp);
            }
        });
    }
    ...
    private void startPrivilegeMonitor() {
        plugin.getSchedulerService().runGlobalAtFixedRate(() -> {
            try {
                // Use snapshot to avoid ConcurrentModificationException on Folia
                ...
                // Folia may throw if called from wrong region; ignore and retry next tick
                plugin.getLogger().warning("[OPProtection] Privilege monitor error: " + t.getMessage());
            }
        }, 20L, 20L); // Reduced from 5 ticks (~250ms) to 20 ticks (1s) for better performance
    }
    ...
    public void handlePasswordLogin(Player player, String input) {
        ...
        // Premium tier auto-bypass: if player is verified premium and bypass is enabled, skip password
        if (plugin.getPreAuthService().isStronglyVerified(player)
                && plugin.getPreAuthService().isPremiumAutoBypassEnabled()) {
            unlockPlayer(player);
            plugin.msg(player, "premium_auto_verified");
            plugin.getAuditLog().write("PREMIUM_AUTO_BYPASS", player.getName(),
                    player.getUniqueId().toString(), ip(player), "Strongly verified premium, password bypassed");
            return;
        }
        UUID uuid = player.getUniqueId();
        ...
        plugin.getAsyncExecutor().execute(() -> {
            ...
            plugin.getSchedulerService().runEntity(player, () -> {
                ...
                if (globalRehash != null) {
                    ...
                    // Persist into config/core.yml only — NEVER use plugin.saveConfig()
                    // because that would flush the entire merged config back into
                    // config.yml and re-introduce the duplication conflict.
                    plugin.getConfigCache().saveCoreValue("op-password", globalRehash);
                }
    ...
    public boolean verify2FACodeInput(Player player, String code) {
        if (player == null || code == null || !player.isOnline()) return false;
        UUID uuid = player.getUniqueId();
    ...
    public void setPassword(Player player, String newPassword) {
        if (player == null || !player.isOnline()) return;
        if (isLocked(player) && !isConfirmed(player)) {
            plugin.msg(player, "op_verification_command_blocked");
            return;
        }
    ...
    /**
     * Invalidate all password rate-limit states to prevent stale lockouts
     * from persisting after a global or personal password change.
     */
    public void invalidateAllRateLimits() {
        passwordLimiter.clear();
        twoFactorLimiter.clear();
    }
    ...
    public void setGlobalPassword(String rawPassword) {
        ...
        // Persist into config/core.yml — see comment in OpManager line ~558.
        plugin.getConfigCache().saveCoreValue("op-password", hash);
        invalidateAllRateLimits();
    }
    ...
    public void handleLogout(Player player) {
        ...
        if (isWhitelisted(player.getName()) && !wasLocked) {
            runLogoutActions(player);
        }
        revokeCurrentPrivileges(player, false);
    }

    /**
     * Execute the configured logout-actions exactly once per session. Both the normal
     * quit path and the verification-timeout path funnel through here; the dedup set
     * prevents double dispatch when a timeout ban/kick is followed by the quit event.
     */
    private void runLogoutActions(Player player) {
        if (player == null || !isWhitelisted(player.getName())) return;
        if (!logoutActionsExecuted.add(player.getUniqueId())) return;
        for (String action : logoutActions) {
            String command = action == null ? "" : action.replace("%player%", player.getName()).trim();
            if (command.isEmpty() || isRedundantPrivilegeRevocation(command, player.getName())) continue;
    ...
    private void resetSession(UUID uuid) {
        ...
        logoutActionsExecuted.remove(uuid);
        generations.computeIfAbsent(uuid, ignored -> new AtomicLong()).incrementAndGet();
    }
    ...
    private void handleVerificationTimeout(Player player) {
        // Run the custom logout-actions here too: a timeout ends the privileged session
        // just like a logout, so configured revocation commands must fire before the
        // ban/kick is applied. runLogoutActions dedups against the quit path.
        runLogoutActions(player);
        ...
        if (action.equals("ban")) {
            ...
            dispatchBan(player.getName(), reason);
        }
        if (player.isOnline()) player.kickPlayer(kickMessage);
    }

    /**
     * Dispatch a console ban with an injection-safe target name. The name is validated
     * against the vanilla username charset and quoted, and control characters are stripped
     * from the reason, so a hostile name or message can never append extra commands.
     */
    private void dispatchBan(String playerName, String reason) {
        if (playerName == null || !SAFE_PLAYER_NAME.matcher(playerName).matches()) {
            plugin.getLogger().severe("[OPProtection] Từ chối ban tên không hợp lệ: "
                    + (playerName == null ? "null" : playerName.replaceAll("[^A-Za-z0-9_]", "?")));
            return;
        }
    ...
    private static boolean constantTimeEquals(String expected, String actual) {
        ...
        // Pad shorter array to equal length to avoid leaking length info via timing
        if (expectedBytes.length != actualBytes.length) {
            int maxLen = Math.max(expectedBytes.length, actualBytes.length);
            byte[] paddedExpected = new byte[maxLen];
            byte[] paddedActual = new byte[maxLen];
            java.util.Arrays.fill(paddedExpected, (byte) 0);
            java.util.Arrays.fill(paddedActual, (byte) 0);
            System.arraycopy(expectedBytes, 0, paddedExpected, 0, expectedBytes.length);
            System.arraycopy(actualBytes, 0, paddedActual, 0, actualBytes.length);
            // Still do constant-time compare, but now length doesn't leak
            boolean lengthMatch = expectedBytes.length == actualBytes.length;
            boolean contentMatch = MessageDigest.isEqual(paddedExpected, paddedActual);
